	...
	*/
	...
	final class SQL extends plugin {
		...
		private function checkPermission() {
			if (!parent::hasPermission('SQL')) {
				parent::logError('Access to SQL-functions denied.', 'SQL');
				return false;
			}
		...
		public function query($qry, $unbuffered = false) {
			...
			if ($mode == 'select') {
				...
				parent::logError('Invalid query.<br />You are only allowed to use SELECT, INSERT, UPDATE, DELETE, CREATE TABLE, ALTER TABLE and DROP TABLE.', 'SQL');
	...
	plugin::init('com.gidix.examplePlugin');
?>
